...
export async function findCategories(manufacturer: string, lang: string = ''): Promise<ContentCategory[]> {
    ...
    return filteredCategories.map((entry) => entry.data);
}
...
async function getFirstCategoryProductImage(manufacturer: string, category_slug: string): Promise<string | null> {
    ...
    if (products.length > 0 && products[0].data.cdn_images.length > 0) {
        return products[0].data.cdn_images[0];
    }
...
export async function getCategoryBySlugWithImage(manufacturer: string, slug: string, lang: string = ''): Promise<ContentCategoryWithImage | undefined> {
    const categories = await findCategories(manufacturer, lang);
    const category = categories.find((entry) => entry.slug === slug);
